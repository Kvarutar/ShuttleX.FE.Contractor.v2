...
switch (process.env.npm_lifecycle_event) {
  case 'ios:dev':
    ...
    exec({ unixCommand: "npx react-native run-ios --scheme 'ShuttleX_Contractor_dev'", winCommand: '' });
    break;
  case 'ios:prod':
    prepareGoogleServices('prod');
    exec({ unixCommand: "npx react-native run-ios --scheme 'ShuttleX_Contractor'", winCommand: '' });
    break;
  case 'android:dev':
    prepareGoogleServices('dev');
    exec({
